import React from 'react';
import { motion, AnimatePresence } from 'motion/react';
import { X, Flame, Terminal, Cpu, Zap, Code } from 'lucide-react';
import { SkillItem } from '../types';
import { audioEngine } from '../utils/AudioEngine';

interface ZanpakutoDrawerProps {
  skill: SkillItem | null;
  onClose: () => void;
}

export const ZanpakutoDrawer: React.FC<ZanpakutoDrawerProps> = ({ skill, onClose }) => {
  if (!skill) return null;

  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex justify-end bg-black/80 backdrop-blur-md">
        <motion.div
          initial={{ x: '100%' }}
          animate={{ x: 0 }}
          exit={{ x: '100%' }}
          transition={{ type: 'spring', damping: 25, stiffness: 200 }}
          className="relative w-full max-w-xl h-full bg-[#08080a] border-l border-red-600/50 p-6 sm:p-10 flex flex-col justify-between overflow-y-auto text-white shadow-[-20px_0_50px_rgba(220,38,38,0.2)]"
        >
          {/* Header */}
          <div>
            <div className="flex justify-between items-start pb-6 border-b border-white/10">
              <div>
                <div className="flex items-center gap-2 text-xs font-mono text-red-500 tracking-widest uppercase mb-1">
                  <Flame size={14} />
                  <span>ZANPAKUTO SPECIFICATION // {skill.category}</span>
                </div>
                <h3 className="text-3xl font-black uppercase text-white tracking-tight">
                  {skill.name}
                </h3>
              </div>
              <button
                onClick={() => {
                  audioEngine.playHover();
                  onClose();
                }}
                className="p-2 text-zinc-400 hover:text-white hover:bg-white/10 transition-colors"
              >
                <X size={20} />
              </button>
            </div>

            {/* Kanji & Release Phrase */}
            <div className="mt-6 p-4 glass-panel border-l-4 border-red-600 bg-red-950/20">
              <div className="text-xs font-mono text-zinc-400 uppercase tracking-widest mb-1 flex justify-between">
                <span>RELEASE COMMAND</span>
                <span className="text-white font-bold">{skill.kanji}</span>
              </div>
              <p className="text-sm font-serif italic text-red-400 font-semibold">
                "{skill.commandPhrase}"
              </p>
            </div>

            {/* Description */}
            <div className="mt-6 space-y-4">
              <p className="text-zinc-300 text-sm leading-relaxed font-light">
                {skill.description}
              </p>

              {/* Proficiency Gauge */}
              <div className="space-y-1.5 pt-2">
                <div className="flex justify-between text-xs font-mono">
                  <span className="text-zinc-400">SPIRITUAL PROFICIENCY RESONANCE</span>
                  <span className="text-red-500 font-bold">{skill.proficiency}%</span>
                </div>
                <div className="w-full h-2 bg-zinc-900 border border-white/10 overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${skill.proficiency}%` }}
                    transition={{ duration: 1 }}
                    className="h-full bg-gradient-to-r from-red-800 to-red-500"
                  />
                </div>
              </div>

              {/* Bankai Form Name */}
              <div className="pt-4">
                <div className="text-xs font-mono text-zinc-400 uppercase tracking-widest mb-1">
                  BANKAI FORM EVOLUTION
                </div>
                <div className="p-3 bg-zinc-900 border border-white/10 text-white font-mono text-xs uppercase tracking-wider text-red-400 font-bold">
                  ⚡ {skill.bankaiForm}
                </div>
              </div>

              {/* Code Implementation */}
              <div className="pt-4">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-xs font-mono text-zinc-400 uppercase tracking-widest flex items-center gap-1.5">
                    <Code size={14} className="text-red-500" />
                    IMPLEMENTATION EXTRACT
                  </span>
                </div>
                <pre className="p-4 bg-black border border-white/10 text-xs font-mono text-red-300 overflow-x-auto leading-relaxed">
                  <code>{skill.codeSnippet}</code>
                </pre>
              </div>
            </div>
          </div>

          {/* Footer Action */}
          <div className="pt-8 border-t border-white/10">
            <button
              onClick={() => {
                audioEngine.playSlash();
                onClose();
              }}
              className="w-full py-3 bg-red-600 hover:bg-white text-white hover:text-black font-black uppercase text-xs tracking-[0.3em] transition-all"
            >
              CLOSE SPECIFICATION
            </button>
          </div>
        </motion.div>
      </div>
    </AnimatePresence>
  );
};
