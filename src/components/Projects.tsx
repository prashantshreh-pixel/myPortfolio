...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, index, bgScaleParallax, onSelect, isLast }) => {
  ...
  return (
    <div style={{ perspective: 1200 }} className="w-full">
      ...
      >
        ...
        {/* Dynamic Holographic Light Glare Overlay */}
        <div
          className="absolute inset-0 pointer-events-none transition-opacity duration-300 z-10"
          style={{
            opacity: glare.opacity,
            background: `radial-gradient(circle 420px at ${glare.x}% ${glare.y}%, rgba(220, 38, 38, 0.22), rgba(255, 255, 255, 0.08) 30%, transparent 70%)`,
          }}
        />
...
export const Projects: React.FC = () => {
  ...
  return (
    <section ref={sectionRef} id="projects" className={`pt-20 sm:pt-24 pb-20 sm:pb-36 relative overflow-hidden ${activeProject ? 'z-50' : 'z-10'}`}>
      ...
      {/* Cinematic Horizontal Cards List with 3D Tilt */}
      <div className="flex flex-col border-y border-white/10 relative z-10">
        {PROJECTS_DATA.map((project, index) => (
          <ProjectCard
            ...
            project={project}
            index={index}
            bgScaleParallax={bgScaleParallax}
